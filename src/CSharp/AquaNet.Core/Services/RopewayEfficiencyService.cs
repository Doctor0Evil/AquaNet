...
namespace AquaNet.Core.Services
{
    ...
    {
        public sealed record RopewayConfig(
            double SpanM,
            double AvgLoadKg,
            double TripTimeS,
            ...
            double FlywheelRoundtripEff,
            double RenewableFraction
        );

        public sealed record RopewayResult(
            double MechEnergyJ,
            double ElecEnergyJ,
            double ElecEnergyKwh,
            double TonKm,
            double KwhPerTonKm,
            double Co2DieselKg,
            double Co2ElecKg,
            double Co2AvoidedKg,
            double EcoScore0To10
        );
        ...
        public RopewayResult SimulateTrip(RopewayConfig cfg)
        {
            if (cfg.SpanM <= 0 ||
                cfg.AvgLoadKg <= 0 ||
                cfg.TripTimeS <= 0 ||
                cfg.CableEfficiency <= 0 || cfg.CableEfficiency > 1 ||
                cfg.MotorEfficiency <= 0 || cfg.MotorEfficiency > 1 ||
                cfg.FlywheelRoundtripEff <= 0 || cfg.FlywheelRoundtripEff > 1 ||
            ...
            const double dragCoeff = 0.8;
            const double areaM2 = 3.0;
            ...
            double kwhPerTonKm = tonKm > 0.0
                ? elecEnergyKwh / tonKm
                : 0.0;

            const double dieselKwhPerTonKm = 0.6;
            const double co2DieselKgPerKwh = 0.27;
            double co2DieselKg = dieselKwhPerTonKm * tonKm * co2DieselKgPerKwh;

            const double gridCo2KgPerKwh = 0.15;
            const double renewableCo2KgPerKwh = 0.02;
            double effCo2Kwh = cfg.RenewableFraction * renewableCo2KgPerKwh +
                               (1.0 - cfg.RenewableFraction) * gridCo2KgPerKwh;
