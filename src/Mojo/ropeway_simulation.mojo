struct RopewayConfig:
    ...
    fn __init__(
        ...
        avg_load_kg: Float64,
        trip_time_s: Float64,
        elevation_diff_m: Float64,
        cable_efficiency: Float64,
        motor_efficiency: Float64,
        flywheel_roundtrip_eff: Float64,
...
fn simulate_trip(cfg: RopewayConfig) -> RopewayResult:
    ...
    let mass_ton = cfg.avg_load_kg / 1000.0
    let distance_km = cfg.span_m / 1000.0
    r.ton_km = mass_ton * distance_km

    let v_avg = cfg.span_m / cfg.trip_time_s
    let drag_coeff = 0.8
    let area_m2 = 3.0
    let rho_air = 1.225

    let f_drag = 0.5 * rho_air * drag_coeff * area_m2 * v_avg * v_avg
    let e_drag = f_drag * cfg.span_m
    ...
    let eff_chain = cfg.motor_efficiency * cfg.flywheel_roundtrip_eff
    ...
    if r.ton_km > 0.0:
        ...
        r.kwh_per_ton_km = 0.0

    let diesel_kwh_per_ton_km = 0.6
    let co2_diesel_kg_per_kwh = 0.27
    r.co2_diesel_kg = diesel_kwh_per_ton_km * r.ton_km * co2_diesel_kg_per_kwh

    let grid_co2_kg_per_kwh = 0.15
    let renewable_co2_kg_per_kwh = 0.02
    let eff_co2_kwh = (
        cfg.renewable_fraction * renewable_co2_kg_per_kwh +
    ...
    let perf = max(0.0, 1.0 - r.kwh_per_ton_km / 1.0)
    let avoid_norm = min(1.0, max(0.0, r.co2_avoided_kg / 10.0))
    r.eco_score_0_10 = 10.0 * 0.6 * perf + 10.0 * 0.4 * avoid_norm

    return r

fn main():
    let cfg = RopewayConfig(
        1200.0,
        40000.0,
        600.0,
        2.0,
